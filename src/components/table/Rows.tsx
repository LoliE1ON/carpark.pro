...
export const Rows = ({ table, EditComponent }: RowsProps) => {
  ...
  const WrapComponent = (WrappedComponent: React.FunctionComponent<WrapEditRecordProps>) => (props: { row: any }) => (
    <WrappedComponent row={props.row} saveRow={saveRow} removeRow={removeRow} />
  );
  ...
  return (
    <>
      {table?.filteredData.map((row: any) => (
        <>
          <div
            className={`table-fox ${table.selectRows.includes(+row.id) ? "table-fox-select" : ""}`}
            onClick={() => toggleRow(+row.id)}
            onDoubleClick={() => onEditRow(+row.id)}
            key={row.id}>
            {table.columns.map(({ name }) =>
              !table.hideColumns.includes(name) ? (
